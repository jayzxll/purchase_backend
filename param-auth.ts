  ...
  async makeSoapRequest(action: string, requestData: any): Promise<any> {
    ...
    for (const soapAction of soapActions) {
      try {
        ...
        this.logSoapRequest(action, soapRequest);

        const timeout = action === 'TP_WMD_UCD' ? 45000 : 30000;
        ...
        const responseText: string = typeof response.data === 'string' ? response.data : String(response.data);
        console.log('✅ Response data (first 500 chars):', responseText.substring(0, 500));

        return this.parseSoapResponse(responseText, action);
        ...
        if (error.response) {
          ...
          console.log(`❌ Server response error (${error.response.status}):`, errorText.substring(0, 500));
  ...
  private buildSoapEnvelope(action: string, requestData: any): string {
    ...
    const buildXmlElement = (key: string, value: any): string => {
      if (value === null || value === undefined || value === '') {
        return `<${key}></${key}>`; // ✅ FIXED: Empty tags instead of no tags
      }
      ...
      if (typeof value === 'object' && !Array.isArray(value)) {
        ...
        return nestedXml ? `<${key}>${nestedXml}</${key}>` : `<${key}></${key}>`;
      } else {
    ...
    // ✅ FIXED: Exact format matching Param's example
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  private logSoapRequest(action: string, soapRequest: string): void {
    console.log('\n================================');
    console.log(`📤 SOAP REQUEST for ${action}`);
    console.log('================================');
    console.log(soapRequest);
    console.log('================================\n');
  }

  compareWithParamExample(): void {
    const ourFormat = this.buildSoapEnvelope('TP_WMD_UCD', {
      G: {
        CLIENT_CODE: '10738',
      ...
      Siparis_ID: 'LEX_20250921_224321_8',
      Siparis_Aciklama: 'testaciklamaAlper',
      Taksit: '1',
      Islem_Tutar: '1000,00',
      Toplam_Tutar: '1000,00',
      Islem_Hash: 'MtttsDVNkbpX7cOwdGJ+yR+hsQ4=',
      Islem_Guvenlik_Tip: '3D',
      Islem_ID: '',
      IPAdr: '127.0.0.1',
